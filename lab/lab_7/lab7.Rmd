---
title: "STAT 33B Lab 7"
author: "Gunnar Mayer (3034535154)"
output: pdf_document
---

This assignment is due __May 1, 2020__ by 11:59pm.
...
The purpose of this lab is to practice profiling and optimizing code.

The function in the next section is quite slow. The exercises in this lab will
walk you through various performance improvements to the function.

**IMPORTANT NOTE 1:** When benchmarking or profiling code, other programs
running on your computer can affect the results. So when you benchmark or
profile code, first make sure you don't have lots of other programs running.
Close your web browser, word processor, media player, etc.

**IMPORTANT NOTE 2:** Each time you profile code the results will vary
slightly, even if you don't change the code. Thus if you are investigating an
optimization that appears to have a small effect, you may want to profile the
code several times to determine whether the effect is real or due to random
variation.

**IMPORTANT NOTE 3:** The code cells in this notebook are all set up with
`eval=FALSE` so that they don't actually run the code they contain. Your
notebook may fail to knit if you put profiler code in a cell that doesn't have
`eval=FALSE`. You do not need to show us the output from profvis; just tell us
...
Use profvis to profile the `sample_markov()` function. Make sure the `n`
argument in your call to `sample_markov()` is large enough that the function
runs for at least 30 seconds. You can use the default arguments for the other
parameters.
...
By preallocating the character vector the run time went from 3330ms to 230ms. This was drastic increase in effiency.
...
YOUR WRITTEN ANSWER GOES HERE:
By changing to a numeric vector run time was further decreased to 180ms. It wasn't as drastic as a difference as preallocation, however, that is around a 30% decrease in run time.
...
If you want to sample 0s and 1s, the `rbinom()` function is significantly
faster. 
...
or 1 with `rbinom()` by setting both the number of observations and the number
of trials to 1. See the documentation for details.

Edit your `sample_markov()` function from round 2 so that it uses `rbinom()` to
sample the 0s and 1s instead of `sample()`. In doing so, it should be possible
to eliminate the variables `prob` and `x`.
...
YOUR WRITTEN ANSWER GOES HERE:
By changing to rbinom the run time decreased even more to 90ms. Exactly half of the the previous time using the sample()  method.
...
compares to the version at the beginning of the lab.
...
profvis({
  ...
  chain = numeric(n)
  chain = runif(chain)
  chain = as.integer(chain < p[1])
  for (i in 1:n) {
    # Check the previous state, chain[i].
    if (chain[i] == 1) {
      # Randomly sample new state using the probabilities.
      chain[i+1] = rbinom(n=1, size=1, prob = p[2])
    }
  }
...
YOUR WRITTEN ANSWER GOES HERE:
My runtime decreased to 70ms by using runif and vector operations. This is slightly better than using rbinom but when comparing this new function to the unaltered initial sample_markov function is amazing. The time difference is 3260 ms between the two. 
